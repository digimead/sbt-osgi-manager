...
public abstract class AbstractIndexedRepo implements RegistryPlugin, Plugin,
		RemoteRepositoryPlugin, IndexProvider, Repository {
	...
	protected AbstractIndexedRepo() {
		...
		// allContentProviders.put(REPO_TYPE_OBR, new
		// ObrContentProvider(obrIndexer));
	...
	protected synchronized void loadAllContentProviders() {
		...
		List<IRepositoryContentProvider> extraProviders = registry
				.getPlugins(IRepositoryContentProvider.class);

		for (IRepositoryContentProvider provider : extraProviders) {
			String providerName = provider.getName();
			if (allContentProviders.containsKey(providerName)) {
				warning("Repository content provider with name \"%s\" is already registered.",
						providerName);
			} else {
	...
	protected final synchronized void init() throws Exception {
		if (!initialised) {
			...
			if (requestedContentProviderList != null
					&& requestedContentProviderList.length() > 0) {
				...
				while (tokenizer.hasMoreTokens()) {
					...
					IRepositoryContentProvider provider = allContentProviders
							.get(token);
					if (provider == null) {
						warning("Unknown repository content provider \"%s\".",
								token);
					} else {
			...
			IRepositoryIndexProcessor processor = new IRepositoryIndexProcessor() {
				...
				public void processReferral(URI parentUri, Referral referral,
						int maxDepth, int currentDepth) {
					try {
						URI indexLocation = new URI(referral.getUrl());
						try {
							CachingUriResourceHandle indexHandle = new CachingUriResourceHandle(
									indexLocation, getCacheDirectory(),
									connector, (String) null);
							indexHandle.setReporter(reporter);
							readIndex(indexLocation.getPath(), indexLocation,
									new FileInputStream(indexHandle.request()),
									this);
						} catch (Exception e) {
						...
						warning("Invalid referral URL '%s' from parent index '%s': %s",
								referral.getUrl(), parentUri, e);
					}
				}
			...
			for (URI indexLocation : indexLocations) {
				try {
					CachingUriResourceHandle indexHandle = new CachingUriResourceHandle(
							indexLocation, getCacheDirectory(), connector,
							(String) null);
					indexHandle.setReporter(reporter);
					File indexFile = indexHandle.request();
					InputStream indexStream = GZipUtils
							.detectCompression(new FileInputStream(indexFile));
					readIndex(indexFile.getName(), indexLocation, indexStream,
							processor);
				} catch (Exception e) {
					warning("Unable to read index at URL '%s': %s",
							indexLocation, e);
				}
			}
	...
	private URLConnector getConnector() {
		...
		synchronized (this) {
			connector = registry != null ? registry
					.getPlugin(URLConnector.class) : null;
		}
		if (connector == null)
	...
	protected ResourceHandle[] getHandles(String bsn, String rangeStr)
		...
		if (versionMap == null || versionMap.isEmpty())
			return null;
		List<Resource> resources = narrowVersionsByVersionRange(versionMap,
				rangeStr);
		List<ResourceHandle> handles = mapResourcesToHandles(resources);

		return (ResourceHandle[]) handles.toArray(new ResourceHandle[handles
				.size()]);
	}
	...
	public ResourceHandle getHandle(String bsn, String range,
			Strategy strategy, Map<String, String> properties) throws Exception {
		init();
	...
	public RepositoryPlugin.PutResult put(InputStream stream,
			RepositoryPlugin.PutOptions options) throws Exception {
	...
	static Version getResourceVersion(Resource resource) {
		...
		if (versionObj == null) {
			...
			throw new IllegalArgumentException(
					"Cannot convert to Version from type: "
							+ versionObj.getClass());
		}
	...
	static URI getContentUrl(Resource resource) {
		List<Capability> caps = resource
				.getCapabilities(ContentNamespace.CONTENT_NAMESPACE);
		if (caps == null || caps.isEmpty())
			throw new IllegalArgumentException(
					"Resource has no content capability");

		Object uri = caps.iterator().next().getAttributes()
				.get(ContentNamespace.CAPABILITY_URL_ATTRIBUTE);
		if (uri == null)
			throw new IllegalArgumentException(
					"Resource content has no 'uri' attribute.");
		...
		try {
			...
			throw new IllegalArgumentException(
					"Failed to convert resource content location to a valid URI.",
					e);
		}
	...
	static String getContentSha(Resource resource) {
		...
		Object contentObj = caps.iterator().next().getAttributes()
		...
		throw new IllegalArgumentException("Content attribute is wrong type: "
				+ contentObj.getClass().toString() + " (expected String).");
	}

	private void readIndex(String name, URI baseUri, InputStream stream,
			IRepositoryIndexProcessor listener) throws Exception {
		...
		for (Entry<String, IRepositoryContentProvider> entry : allContentProviders
				.entrySet()) {
			...
			try {
				...
				checkResult = provider.checkStream(name, new ProtectedStream(
						bufferedStream));
			} finally {
		...
		if (selectedProvider == null) {
			if (maybeSelectedProvider != null) {
				...
				throw new IOException(
						"No content provider understands the specified index.");
			}
		}

		// Finally, parse the damn file.
		try {
			selectedProvider.parseIndex(bufferedStream, baseUri, listener,
					logService);
		} finally {
	...
	static List<Resource> narrowVersionsByFilter(String pkgName,
			SortedMap<Version, Resource> versionMap, Filter filter) {
	...
	static List<Resource> narrowVersionsByVersionRange(
			SortedMap<Version, Resource> versionMap, String rangeStr) {
		List<Resource> result;
		if ("latest".equals(rangeStr)) {
			Version highest = versionMap.lastKey();
			result = Create.list(new Resource[] { versionMap.get(highest) });
		} else {
			VersionRange range = rangeStr != null ? new VersionRange(rangeStr)
					: null;
			...
			for (Entry<Version, Resource> entry : versionMap.entrySet()) {
				...
				if (range != null && range.isRange()
						&& version.compareTo(range.getHigh()) >= 0)
					break;
			}
		}
		return result;
	...
	List<ResourceHandle> mapResourcesToHandles(Collection<Resource> resources)
			throws Exception {
	...
	ResourceHandle mapResourceToHandle(Resource resource) throws Exception {
		...
		try {
			handle = new CachingUriResourceHandle(getContentUrl(resource),
					getCacheDirectory(), getConnector(),
					getContentSha(resource));
		} catch (FileNotFoundException e) {
			throw new FileNotFoundException("Broken link in repository index: "
					+ e.getMessage());
		}
	...
	ResourceHandle resolveBundle(String bsn, String rangeStr, Strategy strategy)
			throws Exception {
		if (rangeStr == null)
			rangeStr = "0.0.0";
	...
	ResourceHandle findExactMatch(String identity, String version,
			Map<String, SortedMap<Version, Resource>> resourceMap)
			throws Exception {
		Resource resource;
	...
	static Resource findVersion(Version version,
			SortedMap<Version, Resource> versions) {
	 ...
	 */
	protected static List<URI> parseLocations(String locationsStr)
	...
	public File get(String bsn, Version version,
			...
			RepositoryPlugin.DownloadListener... listeners) throws Exception {
		init();
		aQute.bnd.service.ResourceHandle handle = resolveBundle(bsn,
				version.toString(), Strategy.EXACT);
		if (handle == null)
			return null;
