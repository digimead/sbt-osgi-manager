...
public class BndrunResolveContext extends ResolveContext {
	...
	private void loadSystemCapabilitiesExtra() {
		String header = (String) runModel
				.genericGet(Constants.RUNSYSTEMCAPABILITIES);
		if (header != null) {
			Processor processor = new Processor();
			try {
				processor.setProperty(Constants.RUNSYSTEMCAPABILITIES, header);
				String processedHeader = processor
	...
	private void loadRepositories() {
		...
		if (repoNames == null) {
			...
			Map<String, Repository> repoNameMap = new HashMap<String, Repository>(
					allRepos.size());
	...
	private void loadEffectiveSet() {
		String effective = (String) runModel
				.genericGet(RUN_EFFECTIVE_INSTRUCTION);
		if (effective == null)
			effectiveSet = null;
		else {
			effectiveSet = new HashSet<String>();
			for (Entry<String, Attrs> entry : new Parameters(effective)
					.entrySet())
	...
	private void findFramework() {
		...
		if (params.size() > 1)
			throw new IllegalArgumentException(
					"Cannot specify more than one OSGi Framework.");
		...
		Filter filter = new SimpleFilter(IdentityNamespace.IDENTITY_NAMESPACE,
				identity);
		if (versionStr != null)
			filter = new AndFilter().addChild(filter).addChild(
					new LiteralFilter(Filters.fromVersionRange(versionStr)));
		Requirement frameworkReq = new CapReqBuilder(
				IdentityNamespace.IDENTITY_NAMESPACE).addDirective(
				Namespace.REQUIREMENT_FILTER_DIRECTIVE, filter.toString())
				.buildSyntheticRequirement();

		// Iterate over repos looking for matches
		for (Repository repo : repos) {
			Map<Requirement, Collection<Capability>> providers = repo
					.findProviders(Collections.singletonList(frameworkReq));
			...
			if (frameworkCaps != null) {
				for (Capability frameworkCap : frameworkCaps) {
					if (findFrameworkContractCapability(frameworkCap
							.getResource()) != null) {
						Version foundVersion = toVersion(frameworkCap
								.getAttributes()
								.get(IdentityNamespace.CAPABILITY_VERSION_ATTRIBUTE));
						if (foundVersion != null) {
							if (frameworkResourceVersion == null
									|| (foundVersion
											.compareTo(frameworkResourceVersion) > 0)) {
								frameworkResource = frameworkCap.getResource();
								frameworkResourceVersion = foundVersion;
								frameworkResourceRepo = new FrameworkResourceRepository(
										frameworkResource, ee, sysPkgsExtra,
										sysCapsExtraParams);
							}
						}
					}
	...
	public static boolean isInputRequirementResource(Resource resource) {
		Capability id = resource.getCapabilities(
				IdentityNamespace.IDENTITY_NAMESPACE).get(0);
		return IDENTITY_INITIAL_RESOURCE.equals(id.getAttributes().get(
				IdentityNamespace.IDENTITY_NAMESPACE));
	}

	private static Version toVersion(Object object)
		...
		throw new IllegalArgumentException(MessageFormat.format(
				"Cannot convert type {0} to Version.", object.getClass()
						.getName()));
	}

	private static Capability findFrameworkContractCapability(Resource resource) {
		List<Capability> contractCaps = resource
				.getCapabilities(ContractNamespace.CONTRACT_NAMESPACE);
		if (contractCaps != null)
			for (Capability cap : contractCaps) {
				if (CONTRACT_OSGI_FRAMEWORK.equals(cap.getAttributes().get(
	...
	public Collection<Resource> getMandatoryResources() {
		...
		if (frameworkResource == null)
			throw new IllegalStateException(MessageFormat.format(
					"Could not find OSGi framework matching {0}.",
					runModel.getRunFw()));
	...
	public List<Capability> findProviders(Requirement requirement) {
		...
		// The selected OSGi framework always has the first chance to provide
		// the capabilities
		if (frameworkResourceRepo != null) {
			Map<Requirement, Collection<Capability>> providers = frameworkResourceRepo
		...
		// Next find out if the requirement is satisfied by a capability on the
		// same resource
		Resource resource = requirement.getResource();
		if (resource != null) {
			List<Capability> selfCaps = resource.getCapabilities(requirement
					.getNamespace());
			if (selfCaps != null) {
			...
			if (capabilities != null && !capabilities.isEmpty()) {
				for (Capability capability : capabilities) {
					if (!result.contains(capability)
							&& isPermitted(capability.getResource())) {
		...
		if (Namespace.RESOLUTION_OPTIONAL.equals(requirement.getDirectives()
			...
			// Only return the framework's capabilities when asked for optional
			// resources.
			List<Capability> fwkCaps = new ArrayList<Capability>(
					listResult.size());
			...
			// If the framework couldn't provide the requirement then save the
	...
	private boolean isPermitted(Resource resource) {
		...
		List<Capability> idCaps = resource
				.getCapabilities(IdentityNamespace.IDENTITY_NAMESPACE);
		if (idCaps == null || idCaps.isEmpty()) {
			log.log(LogService.LOG_ERROR,
					"Resource is missing an identity capability (osgi.identity).");
			return false;
		}
		if (idCaps.size() > 1) {
			log.log(LogService.LOG_ERROR,
					"Resource has more than one identity capability (osgi.identity).");
			return false;
		}
		String identity = (String) idCaps.get(0).getAttributes()
	...
	public int insertHostedCapability(List capabilities, HostedCapability hc) {
		...
		for (int i = 0; i < caps.size(); i++) {
			...
			int otherPriority = otherPrioObj != null ? otherPrioObj.intValue()
					: 0;
