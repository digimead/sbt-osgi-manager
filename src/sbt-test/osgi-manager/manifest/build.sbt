...
InputKey[Unit]("check") := {
  if (!new File("/tmp/MANIFEST.MF").exists) error("/tmp/MANIFEST.MF is not exists")
  ()
}

InputKey[Unit]("checkNot") := {
  if (new File("/tmp/MANIFEST.MF").exists) error("/tmp/MANIFEST.MF is exists")
  ()
}

InputKey[Unit]("delete") := {
  new File("/tmp/MANIFEST.MF").delete()
  ()
}
